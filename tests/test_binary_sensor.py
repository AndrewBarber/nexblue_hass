...
@pytest.mark.asyncio
async def test_binary_sensor_setup_entry_no_chargers(mock_coordinator, mock_config_entry):
...
def test_binary_sensor_charging_idle(mock_coordinator, mock_config_entry):
    ...
    sensor = NexBlueBinarySensor(mock_coordinator, mock_config_entry, "test123", description)
...
def test_binary_sensor_vehicle_connected_disconnected(mock_coordinator, mock_config_entry):
...
def test_binary_sensor_get_charger_data_not_found(mock_coordinator, mock_config_entry):
    ...
    description = BINARY_SENSOR_TYPES[0]
    sensor = NexBlueBinarySensor(mock_coordinator, mock_config_entry, "nonexistent", description)
...
def test_binary_sensor_device_info(mock_coordinator, mock_config_entry):
    ...
    assert device_info["sw_version"] is None  # firmware_version is in status, not at top level
...
def test_binary_sensor_charging_states_comprehensive(mock_coordinator, mock_config_entry):
