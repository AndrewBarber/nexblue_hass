...
from custom_components.nexblue_hass.switch import async_setup_entry, NexBlueChargingSwitch
...
def test_switch_is_on_with_recent_turn_on_command(mock_loop, mock_coordinator, mock_config_entry):
    ...
    mock_coordinator.data["chargers"][0]["status"]["charging_state"] = 1  # Not in normal on states
    assert switch.is_on is True


@patch('asyncio.get_event_loop')
def test_switch_is_on_with_recent_turn_on_command_idle(mock_loop, mock_coordinator, mock_config_entry):
...
@patch('asyncio.get_event_loop')
def test_switch_is_on_with_recent_turn_off_command(mock_loop, mock_coordinator, mock_config_entry):
...
@patch('asyncio.get_event_loop')
def test_switch_is_on_with_recent_turn_off_command_not_charging(mock_loop, mock_coordinator, mock_config_entry):
    ...
    mock_coordinator.data["chargers"][0]["status"]["charging_state"] = 1  # not charging (not 2)
    # The logic on line 100: elif self._last_command == "turn_off" and charging_state != 2:
...
@patch('asyncio.get_event_loop')
@patch('asyncio.sleep')
async def test_async_turn_on(mock_sleep, mock_loop, mock_coordinator, mock_config_entry):
...
@patch('asyncio.get_event_loop')
@patch('asyncio.sleep')
async def test_async_turn_off(mock_sleep, mock_loop, mock_coordinator, mock_config_entry):
...
@patch('asyncio.get_event_loop')
@patch('asyncio.sleep')
async def test_async_turn_on_with_kwargs(mock_sleep, mock_loop, mock_coordinator, mock_config_entry):
...
@patch('asyncio.get_event_loop')
@patch('asyncio.sleep')
async def test_async_turn_off_with_kwargs(mock_sleep, mock_loop, mock_coordinator, mock_config_entry):
